...
def login():
    ...
    return render_template('login_advanced.html', form=form)
...
@login_required
def index():
    return render_template('index_advanced.html', server_status=server_status, server_properties=server_properties)
...
def run_server():
    ...
    for line in server_process.stdout:
        # Use socketio.emit instead of emit
        socketio.emit('server_output', {'output': line.strip()})
...
def start_server():
    ...
    # Let Flask-SocketIO handle threading
...
def configure():
    ...
    return render_template('configure_advanced.html', form=form, server_properties=server_properties)
